  ...
  try {
    ...
    if (req.user?.role === 'SUPERADMIN') {
      if (req.query.tenantId) {
        ...
        const defaultTenant = await prisma.tenant.findUnique({ where: { slug: 'astraonline1' } });
        if (defaultTenant) {
...
export const removeFavicon = async (req: Request, res: Response) => {
  try {
    ...
    if (settings.faviconUrl) {
      ...
      const filePath = path.join(uploadDir, path.basename(settings.faviconUrl.replace('/api/uploads/', '')));
...
export const removeLogo = async (req: Request, res: Response) => {
  try {
    ...
    if (settings.logoUrl) {
      // Remover arquivo físico
      const filePath = path.join(uploadDir, path.basename(settings.logoUrl.replace('/api/uploads/', '')));
      if (fs.existsSync(filePath)) {
        fs.unlinkSync(filePath);
      }
    }
...
export const removeIcon = async (req: Request, res: Response) => {
  try {
    ...
    if (settings.iconUrl) {
      // Remover arquivo físico
      const filePath = path.join(uploadDir, path.basename(settings.iconUrl.replace('/api/uploads/', '')));
      if (fs.existsSync(filePath)) {
        fs.unlinkSync(filePath);
      }
    }
